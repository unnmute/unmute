...
import { generateAlias } from "@/lib/session-alias"
...
export async function POST(request: Request) {
  ...
  const roomAlias = generateAlias(session.id)
  const { data: sessionWithAlias, error: aliasError } = await supabase
    .from("sessions")
    .update({ room_alias: roomAlias })
    .eq("id", session.id)
    .select()
    .single()
  ...
  return NextResponse.json({ session: sessionWithAlias })
}
